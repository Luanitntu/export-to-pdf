...
< !-- <input type="hidden" value="<?php // echo $max[0]->max; ?>" id="max">--><script>jQuery(document).ready(function() {
    ...
    < !-- <audio controls id="bg_audio">
        <source src="<?php // echo bloginfo('template_directory'); ?>/lib/ring.mp3" type="audio/mpeg"></audio>--><form action="" method="post">
            <div class="action"><input type="submit" value="Delete" name="submit" id="doaction" class="button action" onclick="return confirm('Are you sure you want to delete this item?');">
                <?php $status=$wpdb->get_results("SELECT * From  wp_order_status", OBJECT);
...
?>
            ...
            <table style='width:99%; margin-top:20px;' class='wp-list-table widefat fixed striped posts'>
                <thead>
                    <tr style='width:100%; '>
                        ...
                        <td style='width:100px;'>Shipping Type</td>
    ...
    ?>
                <tr style='width:100%; border:1px solid #000;  margin-top: 20px; overflow: hidden;'>
                    <td style='width:10px;' class="<?php echo 'status'.$row->order_status; ?> check-btn"><input type="checkbox" value="<?php echo $row->id;?>" name="choose[]" class="check-list"><span></span></td>
                    <td style='width:20px;'>
                        <?php echo $row->id;
    ?>
                    </td>
    ...
    ?>
                    ...
                    <td style='width:100px;'>
                        <?php echo $date->format('M d, Y - H:i:s'); // $row->created; ?>
                    </td>
                    <td>
...
?>
        ...
        <div class="tablenav bottom">
            < !-- <div class="alignleft actions bulkactions"><label for="bulk-action-selector-bottom" class="screen-reader-text">Select bulk action</label><select name="action2" id="bulk-action-selector-bottom">
                    <option value="-1">Bulk Actions</option>
                    <option value="edit" class="hide-if-no-js">Edit</option>
                    <option value="trash">Move to Trash</option>
                </select><input id="doaction2" class="button action" value="Apply" type="submit">
        </div>--><div class="alignleft actions"></div>
        <?php if($count_page > 1): ?>
...
?>items</span>
            <?php if($paged==1) {
    ?><span class="pagination-links"><span class="tablenav-pages-navspan" aria-hidden="true">«</span><span class="tablenav-pages-navspan" aria-hidden="true">‹</span>
                <?php
}
